...
const Journals = ({ id = 10 }) => {
  ...
  // Fetch trip data using the provided useEffect pattern
  useEffect(() => {
  ...
  // Process trip data into journal entries once trip is loaded
  useEffect(() => {
    if (trip) {
      const journalEntries = processTripsToEntries(trip);
      setEntries(journalEntries);
    }
  }, [trip]);

  // Process trip into journal entries
  const processTripsToEntries = (trip) => {
    const journalEntries = [];

    // Create first entry about discovering the place
    journalEntries.push({
      date: formatDate(trip.createdAt),
      tagline: `Discovered ${trip.name}`,
      content: [
        `Dear Travel Journal,`,
        trip.description,
        `This ${trip.type.toLowerCase()} has ${trip.beds} beds and ${
          trip.baths
        } ${trip.baths === 1 ? "bathroom" : "bathrooms"}, with approximately ${
          trip.square_feet
    ...
    // Create second entry about amenities
    const amenitiesDate = new Date(trip.createdAt);
    amenitiesDate.setDate(amenitiesDate.getDate() + 1);

    journalEntries.push({
      date: formatDate(amenitiesDate),
      tagline: `Exploring ${trip.name}`,
      content: [
        `Dear Travel Journal,`,
        `Today I explored all the amazing amenities at ${trip.name}. ${
          trip.amenities && trip.amenities.length
            ? `The property offers ${trip.amenities.join(", ")}.`
            : "The property offers several nice amenities."
        }`,
        `The location is perfect for travelers looking to experience the local culture and attractions.`,
      ],
      tripId: trip._id,
    });

    // Create third entry about final thoughts
    const finalDate = new Date(trip.updatedAt);
    journalEntries.push({
      date: formatDate(finalDate),
      tagline: `Final thoughts on ${trip.name}`,
      content: [
        `Dear Travel Journal,`,
        `My stay at ${
          trip.name
        } is coming to an end. It's been a wonderful experience in this ${trip.type.toLowerCase()}.`,
        `I would definitely recommend this property to other travelers looking for a comfortable and convenient place to stay.`,
      ],
      tripId: trip._id,
    });
  ...
  useEffect(() => {
    if (timeRef.current) {
      ...
      const timeInterval = setInterval(() => setTime(timeRef.current), 5000);
  ...
  useEffect(() => {
    if (!loading && entries.length > 0 && contentRef.current) {
      const articleElements = contentRef.current.querySelectorAll("article");

      // Reset any existing ScrollTrigger instances
      ScrollTrigger.getAll().forEach((trigger) => trigger.kill());

      articleElements.forEach((article, index) => {
        roll(contentRef.current, article, articleElements, index);
      });

      // Handle scroll limit
      const handleScroll = () => {
        if (contentRef.current.scrollTop > 2550) {
          contentRef.current.scrollTop = 2550;
        }
      };

      contentRef.current.addEventListener("scroll", handleScroll);

      // Keep same progress after resize
  ...
  return (
    <div id="phone" ref={phoneRef}>
      ...
      <svg className="battery" viewBox="0 0 25 12">
        <rect
          opacity="0.35"
          x="0.5"
          y="0.833374"
